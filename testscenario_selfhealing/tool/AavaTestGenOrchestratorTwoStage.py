...
class AavaTestGenOrchestratorTwoStage(BaseTool):
    ...
    def _run(self, **kwargs) -> str:
        ...
        if cfg["stage"] != "testcases":
            ...
            # Banned terms were only ever checked on test cases. Title and description are
            # the generator's own words; the four Ref fields quote the story and are exempt.
            scenbanned = sorted({t for sc in scenarios for t in cfg["bannedterms"]
                                 if has_banned(f"{sc.get('title', '')} "
                                               f"{sc.get('description', '')}", t)})
            if scenbanned:
                log.line("scenbanned", terms=",".join(scenbanned))
            ...
            if cfg["scenarioreviewagentid"] > 0:
                for srnd in (1, 2):
                    ...
                    if scenbanned:
                        fb += (" Remove these banned terms from every title and "
                               "description: " + ", ".join(scenbanned))
                    raw = ""
                    try:
