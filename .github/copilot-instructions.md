...
- `pat_token`: AAVA PAT bearer token, supplied by the caller at tool-execution time
  (required field on `RestApiFormDataCallerSchema` in `wf_caller_tool.py`).
...
- **PAT handling (updated):**
  - `wf_caller_run.py` (standalone) reads the PAT from a `.env` file via the `AAVA_TOKEN`
    environment variable.
  - `wf_caller_tool.py` (container / AAVA) now takes the PAT as a required `pat_token`
    input on `RestApiFormDataCallerSchema`, passed in at tool-execution time by the
    caller/agent instead of being hardcoded. Do not hardcode the token back into the
    script.
