...
check("stage defaults to all", tool._config(json.dumps(base))["stage"] == "all")
...
res20b = json.loads(tool._run(runinputs=json.dumps(
    {k: v for k, v in scen_base.items() if k != "githubtoken"})))
check("a stage run without githubtoken fails at validation",
      res20b.get("status") == "failed" and "githubtoken" in res20b.get("error", ""),
      str(res20b.get("error")))

# ── scenarios stage: review rejects once, one rework, handoff written ────────
_gh18 = {"puts": [], "gets": 0}


def stage_http(method, url, log, headers=None, json_body=None, timeout=None, form=None):
    if method == "GET":
        _gh18["gets"] += 1
        return 404, {"message": "Not Found"}          # first write, no previous sha
    _gh18["puts"].append((url, json_body))
    return 201, {"content": {"path": url.split("/contents/")[-1]}}
...
def stage_exec(agentid, userinputs, cfg, token, budget, log, label):
    if agentid == cfg["scenarioagentid"]:
        _srev["gen"] += 1
        return real_scen, 10
    _srev["n"] += 1
    assert "reviewinputs" in userinputs, "scenario reviewer must get ONE reviewinputs blob"
    if _srev["n"] == 1:
        return json.dumps({"confidence": 60, "approved": False,
                           "feedback": "TS_001 and TS_002 differ only by date, collapse them",
                           "strengths": [], "gaps": ["collapse TS_001/TS_002"]}), 10
    return json.dumps({"confidence": 88, "approved": True, "feedback": "solid coverage",
                       "strengths": ["all AC covered"], "gaps": []}), 10


T.exec_agent, T.fetch_story, T._secret = stage_exec, fake_story, lambda k, f="": f or "tok"
T._http = stage_http
res21 = json.loads(tool._run(runinputs=json.dumps(scen_base)))
check("scenarios stage completes and says which stage it is",
      res21.get("status") == "completed" and res21.get("stage") == "scenarios",
      str({k: res21.get(k) for k in ("status", "stage", "error")}))
check("a rejected review triggers exactly one rework and a second review",
      _srev["n"] == 2 and _srev["gen"] == 2, str(_srev))
check("the envelope carries the verdict and the scenario list",
      res21.get("review", {}).get("approved") is True and len(res21.get("scenarios", [])) > 0)
check("the handoff lands at the stable story-derived path",
      len(_gh18["puts"]) == 1 and
      _gh18["puts"][0][0].endswith("/contents/scenarios/640764/scenarios.json"),
      str([u for u, _ in _gh18["puts"]]))
_handoff = json.loads(_b64.b64decode(_gh18["puts"][0][1]["content"]).decode())
check("the handoff file carries story, scenarios and review",
      set(_handoff) >= {"story", "scenarios", "review"}
      and bool(_handoff["story"].get("title")))

# ── testcases stage: reads the handoff, never calls azure devops ─────────────
_scenlist = T.parse_scenarios(real_scen, 20)
_handoff_b64 = _b64.b64encode(json.dumps(
    {"story": fake_story(None, None, None), "scenarios": _scenlist,
     "review": None}).encode()).decode()


def tc_http(method, url, log, headers=None, json_body=None, timeout=None, form=None):
    if method == "GET" and "/contents/scenarios/640764/scenarios.json" in url:
        return 200, {"content": _handoff_b64, "sha": "abc123"}
    return 404, {"message": "Not Found"}


def ado_boom(cfg, pat, log):
    raise AssertionError("the testcases stage must never call azure devops")


T.exec_agent, T.fetch_story = fake_exec, ado_boom
T._http = tc_http
res22 = json.loads(tool._run(runinputs=json.dumps(tc_base)))
check("testcases stage completes without touching azure devops",
      res22.get("status") == "completed",
      str({k: res22.get(k) for k in ("status", "stage", "error")}))
check("scenario count and workers come from the handoff file",
      res22["summary"]["scenarios"] == len(_scenlist),
...
res23 = json.loads(tool._run(runinputs=json.dumps(tc_base)))
check("a missing handoff file names the fix",
      res23.get("status") == "failed" and res23.get("stage") == "handoff"
      and "run the scenarios stage first" in res23.get("error", ""),
      str(res23.get("error")))

# ── the new prompt's wiring, same rule as section 10 ─────────────────────────
_p04 = open(os.path.join(AGENTS_LOCAL, "04_scenario_reviewer_agent.md"),
            encoding="utf-8").read()
check("04 scenario reviewer: declares exactly one reviewinputs placeholder",
      set(_re.findall(r"\{\{(\w+)\}\}", _p04)) == {"reviewinputs"},
      str(sorted(set(_re.findall(r"\{\{(\w+)\}\}", _p04)))))
_sent04 = {"scenarios", "storytitle", "storydescription", "acceptancecriteria", "passscore"}
check("04 scenario reviewer: every field the tool sends is documented",
      all(f in _p04 for f in _sent04),
      str(sorted(f for f in _sent04 if f not in _p04)))

check("parse_scenario_verdict rejects a verdict without feedback",
      _try(lambda: T.parse_scenario_verdict('{"confidence": 90, "approved": true}')))
check("parse_scenario_verdict rejects a non-boolean approved",
      _try(lambda: T.parse_scenario_verdict(
          '{"confidence": 90, "approved": "yes", "feedback": "x"}')))
...
# 5. Stage A checks banned terms on title/description and feeds them to the rework.
_gh26 = {"puts": 0}


def gh26(method, url, log, headers=None, json_body=None, timeout=None, form=None):
    if method == "GET":
        return 404, {"message": "Not Found"}
    _gh26["puts"] += 1
...
_banned_scen = json.dumps([dict(sc, description=sc["description"] + " Uses DTP03 for the date.")
                           for sc in T.parse_scenarios(real_scen, 20)])
_sb = {"rev": 0, "feedback": []}


def banned_exec(agentid, userinputs, cfg, token, budget, log, label):
    if agentid == cfg["scenarioagentid"]:
        if "feedback" in userinputs:
            _sb["feedback"].append(userinputs["feedback"])
        return _banned_scen, 10
    _sb["rev"] += 1
    return json.dumps({"confidence": 60 if _sb["rev"] == 1 else 88,
                       "approved": _sb["rev"] > 1, "feedback": "collapse TS_001/TS_002",
                       "strengths": [], "gaps": []}), 10


_h26b = T._http
T.exec_agent, T.fetch_story, T._secret = banned_exec, fake_story, lambda k, f="": f or "tok"
T._http = gh26
res26b = json.loads(tool._run(runinputs=json.dumps(dict(scen_base, bannedterms=["DTP03"]))))
check("stage A logs banned terms found in title/description",
      any("step=scenbanned" in l and "DTP03" in l for l in res26b.get("log", [])),
      str(res26b.get("error") or [l for l in res26b.get("log", []) if "scenarios" in l][:2]))
check("the rework feedback names the banned term",
