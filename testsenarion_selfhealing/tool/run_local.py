...
def main():
    ...
    ap.add_argument("--stage", choices=("all", "scenarios", "testcases"),
                    default=d("stage", "all"),
    ...
    # The testcases stage reads everything from the handoff file; no ADO credential needed.
    if not pat and not a.probe and a.stage != "testcases":
    ...
    runinputs = {
        ...
        "scenarioagentid": a.scenarioagent,
        "scenarioreviewagentid": a.scenarioreviewagent,
        "scenariopassscore": d("scenariopassscore", None, int),
        "testcaseagentid": a.testcaseagent,
    ...
    # The scenarios stage has its own, much shorter summary: the deliverable is the
    # handoff file, not a table.
    if res.get("stage") == "scenarios":
        mins, secs = divmod(int(wall), 60)
        rev = res.get("review")
        print(f"\nStory {a.storyid} — {len(res['scenarios'])} scenarios in "
              f"{mins}m {secs:02d}s")
        if rev:
            print(f"  review: confidence {rev['confidence']}, "
                  f"{'approved' if rev['approved'] else 'NOT approved'}")
            for g in rev.get("gaps") or []:
                print("     gap:", g)
        else:
            print("  review: off (scenarioreviewagentid=0)")
        h = res.get("handoff") or {}
        if h:
            print("  handoff: https://github.com/%s/blob/%s/%s"
                  % (h["repo"], h["branch"], h["path"]))
        pub = res.get("published")
        if pub:
            print("  published: %s" % (pub.get("error")
                  or "https://github.com/%s/tree/%s/%s"
                  % (pub["repo"], pub["branch"], pub["path"])))
        _files(tee, rundir)
        return
