...
4b. **KB-driven inbound file naming [MUST]:** Before writing any file-creation step, open and read `kb_file_naming_edi_834`, match the scenario's state/LOB to its Program and Group ID, and build the inbound file name exactly as that KB defines it (for example `TP028433_E2026155_005046104.834`). Write the LITERAL resolved name in the step — never a placeholder, a generic description ("a unique 834 file"), or an invented Trading Partner ID / Group ID. Resolve a separate file name per state × Program/Group ID combination. If the name cannot be resolved from the KB, flag it as a plain-language data gap in the Description and use the closest validated entry — never fabricate one.
   ...
   **The file name is FIXED, not unique-per-case [MUST].** The SAME resolved literal name appears in every test case that uses the same Program/Group ID — exactly as the programme's manual test cases do. Uniqueness of a submission lives in the interchange control numbers (ISA13/GS06/GE02/IEA02), which the file-preparation step populates uniquely per execution — NEVER in the file name. Do not mutate, increment, or randomize any digit of the resolved name to "make it unique" (`123456789`, `223456789`-style serials are fabrication), and never emit an unresolved date pattern such as `EYYYYJJJ`.
...
- **Baseline process step preservation — PRIMARY case only.** The primary case keeps every applicable step from the PROCESS STEPS section — including 1a (resolved file name), 9a (FACETS SQL with the pinned SSN query), 9b (canonical file/GUID), 9c (TM UI statuses), 10 (DB-72 login as its seven atomic steps), and the Facets client steps 10a–10d (as their fourteen atomic actions) — as its own atomic step. Step 9d is NOT part of this list: it belongs only to a case whose condition is staging-database validation. Variant cases must still traverse the whole flow — file prepared with its resolved literal name, staged, processed, verified in FACETS — but compress the baseline plumbing steps into 3–4 combined steps; they never skip a phase outright, never drop the resolved file name, and never compress the assertion steps their condition exists to make. **Never compressible, in any case:** step 10e's Facets UI field checks.
...
- **The final step is an explicit outcome assertion, never a summary [MUST].** Every test case ends with a step whose Expected Result NAMES the literal outcome its condition was written to prove — the resolved value the rule produced, and where a rule selects between several candidate inputs, WHICH one won. Generic closers — "the file is processed successfully end to end", "all validations pass", "the transaction completes as expected" — are a defect: they assert that the pipeline ran, not that the rule under test produced the right answer. A reader must be able to tell from the last step alone what this test case proved.
...
9d. **EDIStageArchive staging validation — dedicated cases ONLY [MUST].** This step belongs to a test case whose condition IS staging-database validation, and to no other. When the case's condition is such a validation, query ALL ELEVEN of these tables on `crt_72.sql.caresource.corp\crt_72` against the `EDIStageArchive` database — not a representative sample: `Interchange`, `FunctionalGroup`, `TransactionSet`, `Member`, `HealthCoverage`, `MemberSupplementalIdentifiers`, `Address`, `MemberLanguage`, `MemberCOBRelatedEntity`, `MemberDisabilityInformation`, `MemberReportingCategories`. Resolve the exact schema qualification, column names and key relationships for each from the `EDIFECS Full with AUX 834` document in `kb_edi_schema_details_003_large`, and write executable SQL with a concrete expected row condition per table.

    **Every OTHER test case carries NO EDIStageArchive step at all [MUST].** A functional rule case proves its rule through TM UI and the FACETS tables; adding two or three staging-table queries to it validates nothing the dedicated case does not already cover, burns step budget the case needs for its own assertions, and produces a run where the same table is queried inconsistently from one case to the next. Query all eleven or query none.
...
10. Login DB-72 Servername: `crt_72.sql.caresource.corp\crt_72` — select Windows Authentication in the "Authentication:" pulldown, click the Connect button, then click New Query. Use `crt_72` for SQL Server launch, connection, and non-FACETS validation only; `crt_69.sql.caresource.corp\crt_69` is for FACETS database validation steps only. Do not swap these servers, and do not replace other scenario-specific endpoints (TM UI URL, NAS staging/archive paths, Edifecs locations) with either of them.
    ...
    **In the PRIMARY case this login is SEVEN atomic steps, not one [MUST]:** (i) launch SQL Server Management Studio; (ii) select `Database Engine` in the "Server type:" pulldown; (iii) enter the server name `crt_72.sql.caresource.corp\crt_72`; (iv) select that environment from the server list; (v) select `Windows Authentication` in the "Authentication:" pulldown; (vi) click Connect; (vii) click New Query. Each carries its own expected result. A single step reading "connect to SQL Server and open a new query window" is the compression this rule exists to stop.
...
**Steps 10a–10d are FOURTEEN atomic actions in the PRIMARY case [MUST]:** locate the Citrix Workspace icon; launch it; click the latest Facets Certification icon; click the "File" menu; select Open Database; browse to `C:\Caresource\Trizetto\PZB_Files`; enter the product file name in the "File Name" field; click Open; click Subscriber/Family in the sidebar; select Open under the File option; click Search; expand Member and double click SSN; enter the SSN from the 834 file and click Find; click Ok and confirm the member record opens. Each is its own step with its own expected result. Collapsing this walkthrough into one or two generic "open Facets and look up the member" steps is a defect.

10e. **Facets UI field verification [MUST].** A SQL row proves the data landed; it does not prove the user-facing field moved. With the member record open, verify in the UI every field this story's rule is supposed to write — identified from the scenario's `descriptionRef`/`acceptanceCriteriaRef` and from {{domainhints}} — naming each field by its literal Facets label and stating the resolved expected value, not "the correct value". Where the case's condition means a field must NOT move, assert the unchanged value explicitly. These checks are required in EVERY test case that reaches Facets, primary and variant alike: they are the assertion the case exists to make, so the variant compression rule does not apply to them.
