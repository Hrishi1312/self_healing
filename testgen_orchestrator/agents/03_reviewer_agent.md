...
12.Coverage completeness — the scope of this check is the SCENARIO, never the whole story: only clauses inside this scenario's own `acceptanceCriteriaRef` and `description` count, and {{storyac}} is used ONLY to read the full text of the clauses those two fields reference — NEVER as a source of additional requirements. A clause of the story that this scenario does not reference belongs to a different scenario and is NEVER a failure here, no matter how important it looks. Within that scope, this check fails ONLY if you can quote a concrete requirement clause (an enumerated attribute value with no dedicated case, a first-occurrence/repeated-segment condition with no case, a named condition with no case) that has NO test case in the table.
