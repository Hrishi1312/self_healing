...
- {{limits}}    — JSON with: passscore, stepsmin, stepsmax. It carries NO test case count, on purpose: the count is never yours to judge.
...
12.Coverage completeness — the scope of this check is the SCENARIO, never the whole story. When the scenario's `description` ends with a numbered `Conditions to cover:` list, that list IS the scope: this check fails ONLY if a numbered entry on that list has NO test case in the table, and each test case is expected to map to exactly one entry — do not demand coverage beyond the list, and NEVER demand a case count from any source but that list: the list length is the expected count, and a table with one case per listed entry is COMPLETE no matter how small. Only when there is no such list, fall back to clauses inside this scenario's own `acceptanceCriteriaRef` and `description`; {{storyac}} is used ONLY to read the full text of the clauses those fields reference — NEVER as a source of additional requirements. A clause of the story that this scenario does not reference belongs to a different scenario and is NEVER a failure here, no matter how important it looks. Either way, this check fails ONLY if you can quote a concrete requirement (a listed condition, an enumerated attribute value, a first-occurrence condition) that has NO test case in the table.

  Procedure [MUST]: before you flag anything under this check, build the mapping for EVERY numbered entry — `N) -> TC_xxx` — by reading each test case's Name and Description and matching the field, grouping or situation it exercises. The case for `5) Rate Cell changes` is the case whose Name says Rate Cell changes; the case for a named grouping is the case whose Name lists those same fields. Only an entry with NO case in that mapping is a gap. NEVER report a condition as missing against the very case whose Name states that condition — run 640764_105053 failed the Rate Cell case for "missing Rate Cell", the Pregnancy Indicator case for "missing Pregnancy Indicator" and the Maintenance Date case for "missing Maintenance Date", and every one of those flags was wrong. If your mapping covers every entry, this check PASSES for every case, whatever else you noticed.
...
13.Domain-hint consistency — when {{domainhints}} is not `none`, this check fails ONLY if you can quote a segment/loop/element usage in the table that CONTRADICTS a mapping stated in the hints (e.g. the table calls a date by a segment the hints assign to a different date, or references an element name the hints say must not be used). Quote both the offending table text and the hint line it contradicts. Terminology the hints do not mention is NOT a violation.
  ...
  **Only the MAPPINGS in the hints are in scope [MUST].** Hints may also carry directives about coverage breadth, required combinations or step depth. Those are addressed to the scenario generator and to `limits`, never to you — out of scope for this check and every other check. Never fail a test case by quoting a coverage or depth sentence from the hints, and never demand a case for a combination this scenario's `Conditions to cover:` list does not contain. In particular, RELEVANT FIELDS, NAMED GROUPINGS and any other enumeration in the hints are addressed to the scenario generator: they NEVER create a coverage requirement here, even when the hints call themselves authoritative and even when the scenario's title mentions relevant fields. Run 640764_105053 failed two finance-regression cases for "missing" a named grouping the scenario's list never contained; both flags were wrong.
