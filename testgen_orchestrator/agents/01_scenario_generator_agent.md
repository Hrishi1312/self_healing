...
7c. **Every scenario ends its `description` with a numbered condition list [MUST].** The last part of each scenario's `description` field is a line-numbered list in this exact form:
   ...
   Each entry is ONE distinct input condition this scenario exists to test — a rule path, an enumerated attribute value (per 7a, listed only in the owning scenario), a boundary, a repeated-occurrence condition (per 7b). The test case generator writes EXACTLY one test case per entry, so this list IS the scenario's test case count: a scenario with two genuine conditions lists two, a scenario expanding a six-value attribute list lists six (one per value, e.g. "3) Aid Category changes while Assessment Date is unchanged"). Do NOT pad the list — never add observability, archival, or traceability entries (those are steps inside a test case, not conditions), and never split one comparison into multiple boundary offsets. The ONE exception is staging-database persistence when the story explicitly asks for it — see the dedicated rule below. Count only what the story's text supports.

   **Never copy {{domainhints}} prose into `description` [MUST].** The hints are working instructions to YOU about how to enumerate; they are not scenario content. Quoting a hint's coverage directive into a scenario's `description` — "coverage follows the manual matrix", "including multi-field combinations per quadrant", "at full atomic depth" — turns a whole-story instruction into a requirement that one scenario is then judged against downstream and can never satisfy, and it deadlocks the repair loop. Read the hints, apply them by writing the right condition entries, and let the `description` carry only this scenario's own conditions in plain business language.

   **Maximize POSITIVE coverage within that bound [MUST].** Before closing a scenario's list, sweep its `descriptionRef`, `acceptanceCriteriaRef`, `dorRef` and `dodRef` once more for every legitimate positive variation the text actually supports — valid data combinations, enumerated attribute values, state/LOB variants, boundary values on the happy path, business-rule permutations — and give each ONE entry. Every entry must trace to explicit business logic, a boundary threshold, an eligibility rule, or a data-validation rule in the story's text; a condition you cannot trace is fabrication and stays out. Within that traceability bound, err toward INCLUDING: omitting a traceable positive condition loses a test case just as surely as padding invents one. Positive conditions should dominate the list naturally — that is the expected shape, not a quota to engineer.

   **Rule-matrix stories get FULL quadrant enumeration [MUST].** When the story's rule turns on the combination of two dimensions — e.g. (a discriminating date changed vs not changed) × (existing/current eligibility vs future/not-yet-begun eligibility) — each quadrant of that matrix is owned by one scenario, and that scenario's `Conditions to cover:` list must enumerate the quadrant's FULL variant set the story's field list supports. **This applies to EVERY quadrant, without exception.** A quadrant listing only its headline rule while its sibling quadrant lists every field variant is an under-enumeration defect, and it is the most common defect in this output — typically the quadrants whose headline rule reads as self-evident get collapsed to one condition. Before you emit, compare the entry counts across the quadrant scenarios: a quadrant with substantially fewer entries than its siblings has been collapsed and must be expanded.

   **The combination set is fixed, not a judgement call [MUST].** Within each quadrant scenario, enumerate exactly: (a) ONE entry per single field in the story's relevant-field list — all of them, never a representative subset; (b) ONE entry per multi-field grouping NAMED in the story text, the acceptance criteria or {{domainhints}}, using that exact grouping; and (c) ONE entry for all relevant fields changing together. Do NOT invent groupings the text does not name — a pair or triple you chose yourself is as much a coverage error as a missing one, because it will not correspond to anything the programme's manual test cases exercise. If the story names no groupings, emit (a) and (c) only, and say so plainly in the scenario `description` so the gap is visible rather than silently filled with guesses. The single-field entries live only in the quadrant scenario that owns that rule outcome (per 7a's ownership rule), never repeated across quadrants with the same expected outcome.

   **Staging-database validation is ONE dedicated scenario, never a step elsewhere [MUST].** When the story's description or acceptance criteria require that inbound data persists to a staging/archive database (for EDI 834 inbound, `EDIStageArchive`), emit ONE scenario that owns that validation, and give its `Conditions to cover:` list one entry per distinct rule path the story requires to be proved persistent — typically the same rule paths the matrix quadrants above own, one entry each. This is how the programme's manual test cases are structured: the staging-table checks live in their own small family of test cases, each proving persistence for one rule path.

   Two consequences, both mandatory. First, **no other scenario's condition list may contain a staging-database entry** — a rule scenario proves its rule through the downstream system of record, not by re-querying staging tables, and scattering partial staging checks across rule scenarios produces a run where the same table is validated inconsistently from one case to the next and some tables are never validated at all. Second, **omitting this scenario silently deletes the entire staging validation family** — the test case generator writes one case per listed condition and cannot invent a case for a condition no scenario declared, so if you do not own it here, nothing downstream covers it.
